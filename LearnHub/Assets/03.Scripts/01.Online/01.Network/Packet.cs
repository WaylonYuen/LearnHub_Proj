...
namespace Network.Packet {
    ...
    public class Send {
        ...
        public void LoginPacket(Player player, DataBaseType databaseType, long ID, string[] Passsword) {
            ...
            //定義封包體的字節數組
            byte[] Password_Byte = new byte[DataBodyLenght + (Passsword.Length * 4)];    //保留每個字串組前4Bytes，以保存每組的字串長度。
            ...
            for (int i = 0; i < Passsword.Length; i++) {
                ...
                BitConverter.GetBytes(IPAddress.HostToNetworkOrder(Temp_Bytes.Length)).CopyTo(Password_Byte, TempIndex);    //計算第i個字組長度存放到TempIndex中
                TempIndex += 4; //右邊往左邊存， 1個int = 4bytes
                Temp_Bytes.CopyTo(Password_Byte, TempIndex);    //存入
                TempIndex += Temp_Bytes.Length; //加上字串組長度，索引至新位置；準備存第二組字串組。
            }

            //將所有資料打包成封包
            byte[] Packet = Build_Package(player.Crccode, PackageType.DataBase, databaseType, DataType.StringData, player.EncryptionMethods, ID, Password_Byte); //組合封包
            Send_Packet(player.Socket, Packet);
        }
        ...
        private byte[] Build_Package(int crcCode, PackageType packageType, DataBaseType databaseType, DataType dataType, EncryptionType encryptionType, long sessionID, byte[] data_Byte) {
            ...
            byte[] packet = new byte[Define_Packet.HeadLength + data_Byte.Length];  //定義封包容器

            /*封裝*/
            crcCode_Byte.CopyTo(packet, Define_Packet.crcCode_SP);                  //int   4 Byte 0~4
            encryptionType_Byte.CopyTo(packet, Define_Packet.encryptionType_SP);    //short 2 Byte 4~6
            sessionID_Byte.CopyTo(packet, Define_Packet.sessionID_SP);              //long  8 Byte 6~14
            bodyLenght_Byte.CopyTo(packet, Define_Packet.bodyLength_SP);            //int   4 Byte 14~18
            dataType_Byte.CopyTo(packet, Define_Packet.dataType_SP);                //short 2 Byte 18~20
            packageType_Byte.CopyTo(packet, Define_Packet.packageType_SP);          //short 2 Byte 20~22
            databaseType_Byte.CopyTo(packet, Define_Packet.databaseType_SP);        //short 2 Byte 22~24           
            data_Byte.CopyTo(packet, Define_Packet.HeadLength);                     //HeadLength ~ [BodyLenght+Headlength]
    ...
    public class Unpack {
        ...
        public int Head_CrcCode(byte[] Head) {
            ...
            Array.Copy(Head, Define_Packet.crcCode_SP, Temp_Byte, 0, 4);  //讀取資料
        ...
        public EncryptionType Head_Encryption(byte[] Head) {
            ...
            Array.Copy(Head, Define_Packet.encryptionType_SP, Temp_Byte, 0, 2); //讀取資料
        ...
        public long Head_SessionID(byte[] Head) {
            ...
            Array.Copy(Head, Define_Packet.sessionID_SP, Temp_Byte, 0, 8); //讀取資料
        ...
        public int Head_BodyLength(byte[] Head) {
            byte[] Temp_Byte = new byte[4];        //建立容器
            Array.Copy(Head, Define_Packet.bodyLength_SP, Temp_Byte, 0, 4);  //讀取資料
        ...
        public DataType Head_DataType(byte[] Head) {
            byte[] Temp_Byte = new byte[2];        //建立容器
            Array.Copy(Head, Define_Packet.dataType_SP, Temp_Byte, 0, 2); //讀取資料
        ...
        public PackageType Head_PackageType(byte[] Head) {
            byte[] Temp_Byte = new byte[2];        //建立容器
            Array.Copy(Head, Define_Packet.packageType_SP, Temp_Byte, 0, 2);  //讀取資料
        ...
        public DataBaseType Head_DataBaseType(byte[] Head) {
            byte[] Temp_Byte = new byte[2];        //建立容器
            Array.Copy(Head, Define_Packet.databaseType_SP, Temp_Byte, 0, 2); //讀取資料
